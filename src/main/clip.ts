...
export function isAutoTagRunning(): boolean {
  return isRunning
}
...
export async function startAutoTag(
  ...
  try {
    ...
    if (total === 0) {
      mainWindow.webContents.send(IPC_CHANNELS.AUTO_TAG_COMPLETE, { folderId, tagged: 0 })
      return
    }
    ...
    mainWindow.webContents.send(IPC_CHANNELS.AUTO_TAG_COMPLETE, { folderId, tagged })
  } finally {
    isRunning = false
  }
}
